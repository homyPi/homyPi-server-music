...
var Raspberry  = require("../../link").getShared().Raspberry;
...
module.exports = function(messager) {
		messager.on("player:status", function(data, topicInfo) {
			...
			if(data.playingId) {
				Playlist.setPlayingId({name: topicInfo.raspberry}, data.playingId);
			}
		});
		messager.on("player:playing:id", function(data) {
			console.log("player:playing:id  " + JSON.stringify(data));
			Playlist.setPlayingId(data._id);
		});
		/*
		socket.on("player:play:track", function(data, topicInfo) {
			if (!topic.raspberry) return;
			var name = topic.raspberry
			var player = Players.get(name);
			if (!player) {
				console.log("id unknown");
				return;
			}
			messager.emit("client:" + name, "player:play:track");
			io.sockets.connected[player.socketId].emit("player:play:track", data.track);
		});
		*/
		/*
		
		socket.on("player:resume", function(data, topicInfo) {
			if (topicInfo.name) {
			messager.emit("player:resume");
		});
